...
app.get("/api/fests", async (req, res) => {
    try {
        const fests = await Fest.find().populate('createdBy', 'name').sort({ startDate: 1 });
        res.json(fests);
    } catch (err) {
...
app.put("/api/fests/:id", authMiddleware, async (req, res) => {
    try {
        ...
        if (fest.createdBy.toString() !== req.user.id) {
            return res.status(403).json({ message: "You are not authorized to update this fest." });
        }

        const updatedFest = await Fest.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
app.delete("/api/fests/:id", authMiddleware, async (req, res) => {
    try {
        ...
        if (fest.createdBy.toString() !== req.user.id) {
            return res.status(403).json({ message: "You are not authorized to delete this fest." });
        }
...
app.post("/api/fests/:id/upvote", authMiddleware, async (req, res) => {
    try {
        ...
        const upvoteIndex = fest.upvotes.findIndex(id => id.toString() === userId);
        ...
        await fest.save();
        const populatedFest = await Fest.findById(fest._id).populate('createdBy', 'name');
